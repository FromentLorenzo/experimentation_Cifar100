...
def create_figure(models_epoch, models_test, colors, model_names):
    ...
    - model_artists : dict(index du modèle -> liste des objets graphiques (lines ou patches))
    - ax_loss, ax_acc, ax_bar : les sous-graphiques pour pouvoir y accéder et mettre à jour leurs légendes
    """
    ...
    for i, data in enumerate(models_epoch):
        ...
        col = colors[i]
        # Lignes train et val (avec label)
    ...
    # --- Graphique 3 : Bar chart pour les résultats de test final ---
...
def main(model_dirs):
    ...
    # 2 lignes : 0 = barre de checkboxes, 1 = zone scrollable de la figure
    main_frame = tk.Frame(root)
    main_frame.pack(fill=tk.BOTH, expand=True)
    main_frame.rowconfigure(1, weight=1)
    main_frame.columnconfigure(0, weight=1)
    
    # ----- Barre des checkboxes en haut (ligne 0) -----
    ...
    # Mettre à jour la zone scrollable quand la taille change
